...
//    //stringPath will not be null if a path exists from a Vertex connected to the 
//    //source all the way to the sink Vertex
//    //BFS will return true if the path that was found moves through a 
//    public static void BFS(Vertex source, Vertex sink, Vertex[] path, boolean[] visitedList,
//            HashMap<String, Boolean> edgeList) {
//        LinkedList<Vertex> queue = new LinkedList<Vertex>();
//        queue.add(source);
//        Vertex currVertex = null;
//        Vertex connectedVertex = null;
//        boolean edgeOrientation = CANNOT_FLOW;
//        String vertexConnection = null;
//
//        while (!queue.isEmpty()) {
//            currVertex = queue.poll();
//
//            for (int i = 0; i < currVertex.getNumberOfConnections(); i++) {
//                connectedVertex = currVertex.getConnection(i);
//                vertexConnection = currVertex.getID() + "," + connectedVertex.getID();
//                edgeOrientation = edgeList.get(vertexConnection);
//                if (edgeOrientation && originalEdge.get(vertexConnection) != null) {
//                    if (!visitedList[connectedVertex.getID()]) {
//                        path[connectedVertex.getID()] = currVertex;
//                        visitedList[connectedVertex.getID()] = true;
//                        queue.add(connectedVertex);
//                    }
//                    if (connectedVertex.equals(sink)) {
//                        foundSink = true;
//                        break;
//                    }
//                }
...
//        }
//        if (foundSink) {
//            String stringPath = sink.toString();
//            currVertex = sink;
//            while (!currVertex.equals(source)) {
//                currVertex = path[currVertex.getID()];
//                stringPath = currVertex.toString() + "," + stringPath;
//            }
//            stringPath = 0 + "," + stringPath;
//            source.setPathToSink(stringPath);
//        }
//    }
//    //Run DFS to find a path from a vertex to the sink using a Residual Graph
//    //The end string must include a different source vertex
//    public static String DFS(Vertex reachedVertex,Vertex source, Vertex secondSource, Vertex sink, String currString,
//            HashMap<String, Boolean> edgeList, boolean[] visited) {
//        if (reachedVertex.equals(source)) {
//            return currString;
//        }
//
//        Vertex currVertex = null;
//        currString += reachedVertex.getID() + ",";
//        String prevString = currString;
//        boolean checkExist = false;
//
//        for (int i = 0; i < reachedVertex.getNumberOfConnections(); i++) {
//            currVertex = reachedVertex.getConnection(i);
//            checkExist = visited[currVertex.getID()];
//            if (!checkExist) {
//                visited[currVertex.getID()] = true;
//                if(secondSource == null) {
//                    secondSource = currVertex;
//                }
//                DFS(currVertex, source, secondSource, sink, currString, edgeList, visited);
//                if (!foundSink) {
//                    if (!prevString.equals(currString)) {
//                        String length = currVertex.getID() + "";
//                        currString = currString.substring(0, currString.length() - (length.length() - 1));
//                    }
//                } else {
//                    break;
//                }
